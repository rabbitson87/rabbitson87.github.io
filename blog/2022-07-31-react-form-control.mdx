...
이렇게만 구현한다면 입력값이 변할때에만 값이 이상있는지 체크한 후 alert를 띄워줄 거에요. 단지 값만 체크해줄뿐 바로 로그인 버튼을 누르거나 2번사항을 개선시키기에는 적절하지가 않네요.
...
이 에러기록을 기반으로 로그인 버튼을 눌러도 에러기록이 있다면 실행되지 않도록 바꿔 볼게요.
