...
function Error메시지({ 이름 }) {
  const { input상태값 } = useContext(Context);
  const { value, error, blur } = input상태값(이름);
  console.log('input상태값',value);
  const [input값, input값변경] = useState(value);
  useEffect(() => {
    input값변경(value);
  },[value]);
