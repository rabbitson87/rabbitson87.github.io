...
function 상태관리함수() {
  ...
  const 이벤트전파 = {
    이벤트들: {},
    이벤트발생: (이름, 데이터) => {
      if (!this.이벤트들[이름]) return;
      this.이벤트들[이름].forEach(callback => callback(데이터))
    },
    구독: (이름, callback) => {
      if (!this.이벤트들[이름]) this.이벤트들[이름] = [];
      this.이벤트들[이름].push(callback);
    },
    구독해지: (이름) => {
      if (!this.이벤트들[이름]) return;
      delete this.이벤트들[이름];
